...
pub fn auto_crop_to_limit(
    ...
    let full_mml = generate_mml_final(voice_notes, bpm, start_octave, compress_mode);
    if full_mml.len() <= char_limit {
        return (
            voice_notes.to_vec(),
            voice_notes.len(),
            voice_notes.last().unwrap().end,
        );
    }

    let mut left = 1;
    let mut right = voice_notes.len();
    let mut best_count = 0;
    let mut best_end_tick = 0;

    while left <= right {
        let mid = (left + right) / 2;
        let cropped = &voice_notes[..mid];
        let mml = generate_mml_final(cropped, bpm, start_octave, compress_mode);

        if mml.len() <= char_limit {
            best_count = mid;
            best_end_tick = cropped.last().unwrap().end;
            left = mid + 1;
        } else {
            right = mid - 1;
        }
...
pub fn crop_voice_to_limit(voice_notes: &[Note], bpm: u32, char_limit: usize, compress_mode: bool) -> Vec<Note> {
    if voice_notes.is_empty() {
        return Vec::new();
    }

    let first_note = voice_notes[0].note;
    let start_octave = ((first_note as i32 / 12) - 1).max(2).min(6);

    let (cropped, _, _) = auto_crop_to_limit(voice_notes, bpm, start_octave, char_limit, compress_mode);
    cropped
}